...
router.put('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (mmMembers.length > 0) {
      const addRes = await db.addMmMembers(
        mmMembers.map(member => ({ ...member, eventId: new ObjectId(eventId) }))
      );
    ...
    return res
      .status(500)
      .json({ ok: false, message: 'Internal server error while updating mm-members' });
  }
});
