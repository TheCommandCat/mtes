...
router.put('/', async (req: Request, res: Response) => {
  ...
  const addRes = await db.addMembers(
    members.map(member => ({ ...member, eventId: new ObjectId(eventId) }))
  );
...
router.put('/:memberId/presence', async (req: Request, res: Response) => {
  ...
  if (typeof isPresent !== 'boolean' && replacedBy === undefined) {
    ...
    return res
      ...
      .json({
        ok: false,
        message: 'isPresent field (boolean) or replacedBy field (string) is required'
      });
  }
  ...
  if (replacedBy && typeof replacedBy !== 'object') {
    ...
    return res
      .status(400)
      .json({ ok: false, message: 'replacedBy field must be a WithId<Member>' });
  }

  let updatePayload: { isPresent: boolean; replacedBy: WithId<Member> | null } = {
    isPresent,
    replacedBy: null
  };
  ...
  if (replacedBy) {
    console.log(
      `⏬ Member ${memberId} is being replaced by ${replacedBy._id}. Setting isPresent to true.`
    );
  ...
  try {
    const memberResult = await db.updateMember(
